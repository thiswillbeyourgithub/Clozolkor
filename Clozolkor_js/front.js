...
<!-- HEADER -->
<span id="decksContainer" style="font-size: 10px"> {{Deck}} </span>
<span id="tagsContainer" style="font-size: 10px"> {{Tags}} </span>
...
<div class="{{Tags}}">
  <span class="ease{{info-Factor:}}">
    ...
    <hr id="answer" />
    <br /><br />
    <span class="extra"> {{hint:hint}} </span>
    {{/hint}}
  </span>
</div>
...
<script>
  ...
  try {
    ...
    try {
      // tries to remove those variables set in the back section that screw everything up
      n = null;
      n = undefined;
      c = null;
      c = undefined;
    ...
    if (clozes.length !== 0) {
      ...
      var defaultVisiFront = [...document.querySelectorAll("*")][0].style
        .visibility;
      ...
      const addStateHereFront =
        document.getElementsByClassName("addStateHereFront");
      ...
      if (autoFlip == "T") {
        if (isOnMobileFront == "T" && isOnAndroidFront == "T") {
          ...
          debug(
            "AnkiMobile detected, I need iOS testers for clozolkor to enable autoflip on ankiMobile!",
          );
        ...
        // don't load the front if the card flips on its own
          ...
          if (decksContainer.childElementCount == 0) {
            ...
            for (
              ...
              decksContainer.querySelectorAll("button")[i].style.color =
                cloze_color;
              decksContainer.querySelectorAll("button")[
                i
              ...
              decksContainer.querySelectorAll("button")[i].style.borderRadius =
                "-1px";
              decksContainer.querySelectorAll("button")[i].style.border =
                "none";
        ...
        if (enableTagsContainerFront == "T") {
          if (tagsContainer.childElementCount == 0) {
            var tagList = tagsContainer.innerHTML
              .replace(/ +/, "")
              .split(/::| /);
